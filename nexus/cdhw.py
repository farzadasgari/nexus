...
cdhw_path = "../dataset/cdhw.csv"

if os.path.exists(cdhw_path):
    print("Loading precomputed data from cdhw.csv...")
    data = pd.read_csv(cdhw_path, low_memory=False, parse_dates=['Date'], index_col='Date')
else:
    ...
    def fit_distributions_per_doy(data, Nd=15, min_pos_values=10):
        ...
        for doy, group in data.groupby('doy'):
            ...
            best_dists_summary.append(f"doy={doy}: best_dist={best_name}")
        print(f"\nBest distributions for SPI (Nd={Nd}):")
        print("\n".join(best_dists_summary))
        return day_params
    ...
    def shi(data, Nh, min_sample=10):
        ...
        for doy in sorted(data['doy'].unique()):
            ...
            for year in years:
                ...
                if best_name is None: continue
                best_dists_summary[doy].append(best_name)
            ...
            if best_dists_summary[doy]:
                from collections import Counter
                dist_counts = Counter(best_dists_summary[doy])
                most_common = dist_counts.most_common(1)[0][0]
                print(f"For SHI (Nh={Nh}), doy={doy}: most common best_dist={most_common} (across {len(best_dists_summary[doy])} windows)")
        return data

    for Nh in Nh_list:
        data = shi(data, Nh=Nh)
    data.to_csv(cdhw_path)
    print(f"Saved computed data to {cdhw_path}")
